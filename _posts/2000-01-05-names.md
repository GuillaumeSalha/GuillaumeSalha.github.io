...
The first part presents the average outcome of a human-robot portrait that we can establish based on `characterDF`. However, choosing **trends** in a movie may not reflect the reality of the human first name distribution. The alien teacher lets slip to his students that he has a baby name dataset of the US population over the years (Plot twist: the teacher knew everything from the beginning but he hid it from his students!).

From this second dataset, it will be possible to compare the first name distribution over the years and see if the movie dataset is representative of reality. It could also help us see if a firstname trend arrives in the movie dataset before real life or vice versa.
...
## Creation of new first names in birth register that come from top 100 movies
...
Examples of three first names where the movie is more likely to have created the first name which was given to a baby after the movie release:
...
For the sake of clarity of what is now in his dataset before conducting a deeper analysis, the alien student decids to plot the **normalized** frequency of some arbitrarily chosen first names in movies and in the birth register. The line about the movies is **smoothed** by the **savgol filter**.
